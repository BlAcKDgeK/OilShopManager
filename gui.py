from PyQt5.QtWidgets import (QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout, QHBoxLayout, QTableWidget,
                             QTableWidgetItem, QPushButton, QLabel, QLineEdit, QMessageBox, QSizePolicy, QComboBox,
...
class OilShopApp(QMainWindow):
    def __init__(self):
        ...
        # Футер (контакты, политика и т.д.)
        footer_label = QLabel(
            "© 2025 Интернет-магазин растительного масла. Все права защищены. | "
            "Контакты: info@oilshop.ru | Телефон: +7 (812) 123-45-67 | "
            "<a href='#privacy' style='color: White; text-decoration: none;'>Политика конфиденциальности</a> | "
            "<a href='#terms' style='color: White; text-decoration: none;'>Условия использования</a>"
        )
        ...
        """)
        footer_label.setOpenExternalLinks(False)
        footer_label.linkActivated.connect(self.on_footer_link_clicked)
    ...
    def on_footer_link_clicked(self, link):
        if link == "#privacy":
            QMessageBox.information(
                self,
                "Политика конфиденциальности",
                "Мы не собираем, не храним и не передаём персональные данные третьим лицам. "
                "Ваши данные используются исключительно для оформления заказов и связи с вами."
            )
        elif link == "#terms":
            QMessageBox.information(
                self,
                "Условия использования",
                "Используя данный сервис, вы соглашаетесь с условиями использования. "
                "Товары предоставляются в соответствии с описанием на сайте. "
                "Возврат возможен в течение 14 дней при сохранении товарного вида."
            )
        else:
            QMessageBox.warning(self, "Ссылка", "Ссылка не распознана.")

    def apply_styles(self):
        self.setStyleSheet("""
            QMainWindow { background-color: #f0f8e7; }
            QTabWidget::pane { border: 1px solid #c0c0c0; background: #ffffff; }
            QTabBar::tab { background: #e8f5e8; padding: 15px 10px; font: bold 12pt; color: #2e7d32; min-width: 120px; 
            min-height: 35px; }
            ...
            QTableWidget { guideline-color: #ddd; background: #fff; }
            QLabel { font: 10pt; }
    ...
    def setup_catalog(self):
        ...
        self.min_price.setRange(0, 3000)
        self.max_price = QSpinBox()
        self.max_price.setRange(0, 3000)
        self.max_price.setValue(3000)
    ...
    def load_products(self, search="", category="", min_p=0, max_p=float('inf')):
        ...
        self.catalog_tab.layout().insertWidget(1, self.progress)
        self.progress.show()
    ...
    def create_product_card(self, product):
        ...
        add_cart_btn.setObjectName("small")
        ...
        self.products_layout.insertWidget(0, card)
    ...
    def setup_admin(self):
        ...
        self.admin_products_table.setHorizontalHeaderLabels(["ID", "Название", "Категория", "Цена", "Описание"])
        ...
        self.admin_orders_table.setHorizontalHeaderLabels(["ID", "Пользователь", "Товар", "Кол-во", "Дата", "Статус"])
    ...
    def update_admin_products(self):
        ...
        table.setHorizontalHeaderLabels(["ID", "Название", "Категория", "Цена", "Описание"])
    ...
    def setup_login(self):
        ...
        for field in [self.login_email, self.login_name, self.login_address]:
            field.setPlaceholderText("Введите данные")
        ...
        for label, field in zip(labels, [self.login_email, self.login_name, self.login_address]):
        ...
        form_container.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Minimum)
